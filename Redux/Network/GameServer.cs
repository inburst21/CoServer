...
namespace Redux.Game_Server
{
    ...
    public unsafe sealed class GameServer
    {
        ...
        public static void ProcessPacket(Player client, byte* ptr, byte[] safePacket, ushort type)
        {
            ...
            if (!IgnoredPacket(type))
                COLogger.StartTimer("Packet " + type);
            switch (type)
            {
                ...
                default: Console.WriteLine("Unknown packet type {0}, {2} from {1} ", type, client.Name == null ? "NO NAME" : client.Name, ((InteractPacket) ptr).Action); break;
            }

            if(!IgnoredPacket(type))
                COLogger.StopTimer("Packet " + type);
 }
        #endregion
        private static bool IgnoredPacket(ushort type)
        {
            return Constants.ignoredPackets.Contains(type);
        }
                ...
                case ItemAction.MeteorUpgrade:
                    {
                        ...
                        if (Common.PercentSuccess(mainItem.ChanceToUpgradeLevel()))
                        {
                            ...
                            if (Common.PercentSuccess(Constants.SOCKET_RATE))// || (subItem.plus == 1 && mainItem.BaseItem.Gem1 == 0))
                            {
                                if (mainItem.Gem1 == 0)
                                {
                                    mainItem.Gem1 = 255;
